...
ControllerServiceWatcher::ControllerServiceWatcher(AllJoynBusAttachment^ busAttachment) :
...
ControllerServiceWatcher::~ControllerServiceWatcher()
{
    UnregisterFromBus();
}

void ControllerServiceWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.LSF.ControllerService" };
...
void ControllerServiceWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.LSF.ControllerService"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.LSF.ControllerService"),
            port);
        Added(this, args);
    }
...
void ControllerServiceWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<ControllerServiceWatcher>
        };
...
void ControllerServiceWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_ControllerServiceIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &ControllerServiceWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void ControllerServiceWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void ControllerServiceWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
