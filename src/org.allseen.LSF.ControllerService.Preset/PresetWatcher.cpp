...
PresetWatcher::PresetWatcher(AllJoynBusAttachment^ busAttachment) :
...
PresetWatcher::~PresetWatcher()
{
    UnregisterFromBus();
}

void PresetWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.LSF.ControllerService.Preset" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.LSF.ControllerService.Preset"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.LSF.ControllerService.Preset"),
            port);
        Added(this, args);
    }
...
void PresetWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<PresetWatcher>
        };
...
void PresetWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_PresetIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &PresetWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void PresetWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void PresetWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
