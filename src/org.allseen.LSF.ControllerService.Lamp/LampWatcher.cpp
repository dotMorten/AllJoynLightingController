...
LampWatcher::LampWatcher(AllJoynBusAttachment^ busAttachment) :
...
LampWatcher::~LampWatcher()
{
    UnregisterFromBus();
}

void LampWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.LSF.ControllerService.Lamp" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.LSF.ControllerService.Lamp"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.LSF.ControllerService.Lamp"),
            port);
        Added(this, args);
    }
...
void LampWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<LampWatcher>
        };
...
void LampWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_LampIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &LampWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void LampWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void LampWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
