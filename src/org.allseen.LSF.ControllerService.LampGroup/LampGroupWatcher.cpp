...
LampGroupWatcher::LampGroupWatcher(AllJoynBusAttachment^ busAttachment) :
...
LampGroupWatcher::~LampGroupWatcher()
{
    UnregisterFromBus();
}

void LampGroupWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.LSF.ControllerService.LampGroup" };
...
void LampGroupWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.LSF.ControllerService.LampGroup"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.LSF.ControllerService.LampGroup"),
            port);
        Added(this, args);
    }
...
void LampGroupWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<LampGroupWatcher>
        };
...
void LampGroupWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_LampGroupIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &LampGroupWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void LampGroupWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void LampGroupWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
